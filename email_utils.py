...
def format_alert_email(payload: Dict[str, Any]) -> str:
    ...
    if family == 'RIDE':
        sp = extras.get('structure_phase_detail') or extras.get('structure_phase') or g('structure_phase','StructurePhase')
        sp_conf = extras.get('structure_phase_confidence')
        sp_interp = extras.get('structure_phase_interpretation')
        if sp:
            if sp_conf is not None:
                try:
                    lines.append(f"Structure phase: {sp} ({float(sp_conf):.2f})")
                except Exception:
                    lines.append(f"Structure phase: {sp} ({sp_conf})")
            else:
                lines.append(f"Structure phase: {sp}")
        if sp_interp:
            lines.append(f"Phase interpretation: {sp_interp}")
    ...
    if extras:
        ...
        for k in [
            ...
            "accept_line",
            "structure_phase",
            "structure_phase_detail",
            "structure_phase_confidence",
            "impulse_quality",
            "disp_ratio",
            "atr_pct",
            "baseline_atr_pct",
