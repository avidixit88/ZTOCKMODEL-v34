...
- Removed the dead duplicate `session_vwap()` definition so the codebase now has a single canonical session VWAP implementation.
- Patched `compute_ride_signal()` so session classification honors the same allow-window toggles passed into the engine.
- Standardized VWAP diagnostics across engine payloads via explicit `vwap_reference_*` metadata so chart-vs-engine audits can see which VWAP basis was active.
- Clarified that SWING uses a cumulative VWAP proxy on resampled 30-minute bars for HTF alignment diagnostics.
