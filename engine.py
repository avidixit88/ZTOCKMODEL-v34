    ...
    *,
    ...
    for sym in symbols:
        try:
            ...
            # Use GLOBAL_QUOTE for display/alert Last when present. This keeps SCALP
            # alert Last aligned with the live-price table/RIDE Last, while candle
            # structure/entry math can still use closed-bar OHLCV.
            try:
                if quote is not None and float(quote) > 0 and np.isfinite(float(quote)):
                    qv = float(quote)
                    res.last_price = qv  # type: ignore
                    if isinstance(getattr(res, "extras", None), dict):
                        res.extras["live_last_price"] = qv
                        res.extras["display_last_price"] = qv
                        res.extras["last_price_source"] = "GLOBAL_QUOTE"
                        try:
                            res.extras["last_bar_price"] = float(ohlcv["close"].iloc[-1])
                        except Exception:
                            pass
            except Exception:
                pass
    ...
    *,
    ...
    for sym in symbols:
        try:
            ...
            # Use one synchronized GLOBAL_QUOTE for every engine's alert/display Last.
            # This prevents SCALP from showing a stale/closed-bar Last while RIDE shows
            # the live quote for the same symbol, especially during extended hours.
            try:
                quote_live = float(quote) if quote is not None and float(quote) > 0 and np.isfinite(float(quote)) else None
            except Exception:
                quote_live = None
            if quote_live is not None:
                try:
                    for _res in (r1, r2, r3, r4):
                        try:
                            _res.last_price = quote_live  # type: ignore
                            if isinstance(getattr(_res, "extras", None), dict):
                                _res.extras["live_last_price"] = quote_live
                                _res.extras["display_last_price"] = quote_live
                                _res.extras["last_price_source"] = "GLOBAL_QUOTE"
                                try:
                                    _res.extras["last_bar_price"] = float(ohlcv["close"].iloc[-1])
                                except Exception:
                                    pass
                        except Exception:
                            pass
