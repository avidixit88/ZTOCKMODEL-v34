...
with st.sidebar.expander("AUTO‑EXEC (E*TRADE) — LONG only", expanded=False):
    ...
    exec_allow_preopen = st.checkbox(
        "Allow auto-exec entries in Pre-open window (09:40–09:50 ET)",
        value=True,
...
refresh_seconds = st.sidebar.slider("Refresh every (seconds)", 10, 180, 10, 5, key="ui_refresh_seconds") if auto_refresh else None
...
with tab_scan:
    ...
    if results_rev:
        ...
        for r in results_rev:
            ...
            _rev_rows.append({
                ...
                # Prefer the synchronized GLOBAL_QUOTE display price when available.
                # Entry/stop/TP still come from SCALP structure; this only prevents
                # stale closed-bar Last values from appearing in alerts/emails.
                "Last": ex.get("display_last_price") or ex.get("live_last_price") or d.get("last_price"),
        ...
        chart_detail_ready = False
        chart_detail_error = None
        with st.spinner(f"Loading chart data for {pick}..."):
            try:
                ...
                chart_detail_ready = True
            except Exception as e:
                chart_detail_error = str(e)

        if not chart_detail_ready:
            st.warning(f"Chart & Signal Detail unavailable for {pick} on this refresh: {chart_detail_error or 'data fetch failed.'}")
            st.caption("The scanner, alerts, and auto-exec remain available; only the detail pane was skipped for this rerun.")
        else:
    ...
    if auto_refresh:
        ...
        time.sleep(float(refresh_seconds or 10))
        st.rerun()
