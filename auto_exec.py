...
class AutoExecConfig:
    ...
    # SCALP reversal-trigger entry flexibility.
    # Applies ONLY to structured SCALP reversal-trigger lifecycles. This prevents the
    # new trigger lane from missing valid first-higher-low / reclaim attempts merely
    # because price is a few bps above the trigger close, while still blocking true chase.
    scalp_reversal_entry_flex_bps: float = 40.0
...
class TradeLifecycle:
    ...
    # --- Engine context preserved for Adaptive Threshold v2 ---
    # These fields are additive and optional. They let threshold exits use the
    # same trader context SCALP/RIDE used for entry, instead of relying only on
    # raw TP0/stop geometry. No outbound payload contract changes are required.
    structure_phase_detail: Optional[str] = None
    structure_phase_confidence: Optional[float] = None
    macd_momentum_state: Optional[str] = None
    macd_soft_caution: bool = False
    pressure_score: Optional[float] = None
    extension_state: Optional[str] = None
    breakout_proximity_bucket: Optional[str] = None
    scalp_reclaim_continuation: bool = False
    scalp_reversal_trigger: bool = False
    scalp_reversal_trigger_type: Optional[str] = None
    scalp_reversal_trigger_score: Optional[float] = None
    scalp_chase_quality: Optional[str] = None
    adaptive_threshold_profile: Optional[str] = None
...
def autoexec_cfg_from_raw(raw: Any) -> AutoExecConfig:
    ...
    coercers = {
        ...
        "scalp_reversal_entry_flex_bps": lambda v: max(0.0, min(_coerce_float(v, 40.0), 75.0)),
...
def lifecycle_from_raw(raw: Any) -> TradeLifecycle:
    ...
    coercers = {
        ...
        "structure_phase_detail": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
        "structure_phase_confidence": lambda v: (_coerce_float(v, None) if v not in (None, "") else None),
        "macd_momentum_state": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
        "macd_soft_caution": lambda v: _coerce_bool(v, False),
        "pressure_score": lambda v: (_coerce_float(v, None) if v not in (None, "") else None),
        "extension_state": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
        "breakout_proximity_bucket": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
        "scalp_reclaim_continuation": lambda v: _coerce_bool(v, False),
        "scalp_reversal_trigger": lambda v: _coerce_bool(v, False),
        "scalp_reversal_trigger_type": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
        "scalp_reversal_trigger_score": lambda v: _coerce_float(v, None),
        "scalp_chase_quality": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
        "adaptive_threshold_profile": lambda v: (str(v).upper().strip() if v not in (None, "") else None),
...
def _adaptive_threshold_context(lifecycle: TradeLifecycle) -> Dict[str, Any]:
    """Collect engine context preserved on the lifecycle for Adaptive Threshold v2."""
    out: Dict[str, Any] = {}
    try:
        out["engine"] = str(getattr(lifecycle, "engine", "") or "").upper()
        out["ride_entry_mode"] = str(getattr(lifecycle, "ride_entry_mode", "") or "").upper()
        out["phase"] = str(getattr(lifecycle, "structure_phase_detail", "") or "").upper()
        out["phase_conf"] = _coerce_float(getattr(lifecycle, "structure_phase_confidence", None), None)
        out["macd_state"] = str(getattr(lifecycle, "macd_momentum_state", "") or "").upper()
        out["macd_soft_caution"] = bool(getattr(lifecycle, "macd_soft_caution", False))
        out["pressure_score"] = _coerce_float(getattr(lifecycle, "pressure_score", None), None)
        out["extension_state"] = str(getattr(lifecycle, "extension_state", "") or "").upper()
        out["breakout_bucket"] = str(getattr(lifecycle, "breakout_proximity_bucket", "") or "").upper()
        out["scalp_reclaim_continuation"] = bool(getattr(lifecycle, "scalp_reclaim_continuation", False))
        out["scalp_reversal_trigger"] = bool(getattr(lifecycle, "scalp_reversal_trigger", False))
        out["scalp_reversal_trigger_type"] = str(getattr(lifecycle, "scalp_reversal_trigger_type", "") or "").upper()
        out["scalp_reversal_trigger_score"] = _coerce_float(getattr(lifecycle, "scalp_reversal_trigger_score", None), None)
        out["scalp_chase_quality"] = str(getattr(lifecycle, "scalp_chase_quality", "") or "").upper()
    except Exception:
        pass
    return out


def _adaptive_threshold_trust_score(
    lifecycle: TradeLifecycle,
    reward_room_pct: float,
    failure_room_pct: float,
) -> tuple[float, str]:
    """Return geometry trust score and a compact reason label.

    High trust means the engine entered early/near real structure and adaptive can
    lean more toward raw geometry. Low trust means geometry may be misleading
    because the entry is late, stretched, tapering, or CHASE-like; adaptive should
    lean more fractional and tighter.
    """
    ctx = _adaptive_threshold_context(lifecycle)
    engine = str(ctx.get("engine") or "").upper()
    mode = str(ctx.get("ride_entry_mode") or "").upper()
    phase = str(ctx.get("phase") or "").upper()
    macd_state = str(ctx.get("macd_state") or "").upper()
    ext_state = str(ctx.get("extension_state") or "").upper()
    bucket = str(ctx.get("breakout_bucket") or "").upper()
    chase_quality = str(ctx.get("scalp_chase_quality") or "").upper()
    phase_conf = ctx.get("phase_conf")
    pressure = ctx.get("pressure_score")
    score = 0.50
    reasons: list[str] = []

    early_phases = {"PRE_COMPRESSION", "COMPRESSION_BUILDUP", "ACCEPTANCE_ATTEMPT", "EARLY_ACCEPTANCE", "RECOVERY_BUILD", "RE_COMPRESSION"}
    strong_phases = {"BREAK_AND_HOLD", "PERSISTENT_CONTINUATION"}
    late_phases = {"MATURE_ACCEPTANCE", "EXTEND_THEN_PULLBACK", "STALLING_EXTENSION", "FAILED_EXTENSION"}
    good_macd = {"IGNITING", "ACCELERATING", "SUSTAINING", "PRE_IGNITION", "BUILDING"}
    bad_macd = {"TAPERING", "ROLLING_OVER", "COUNTER_TREND", "FADING"}

    if engine == "RIDE":
        if phase in early_phases:
            score += 0.18; reasons.append(f"phase={phase}")
        elif phase in strong_phases:
            score += 0.10; reasons.append(f"phase={phase}")
        elif phase in late_phases:
            score -= 0.22; reasons.append(f"late_phase={phase}")
        if mode == "PULLBACK":
            score += 0.08; reasons.append("mode=PULLBACK")
        elif mode == "BREAKOUT":
            score -= 0.02; reasons.append("mode=BREAKOUT")
        elif mode == "MOMENTUM":
            score -= 0.12; reasons.append("mode=MOMENTUM")
        if bucket in {"TIGHT", "NEAR", "FRESH", "INSIDE"}:
            score += 0.08; reasons.append(f"break={bucket}")
        elif bucket in {"FAR", "EXTENDED", "CHASE"}:
            score -= 0.12; reasons.append(f"break={bucket}")
    elif engine == "SCALP":
        if bool(ctx.get("scalp_reclaim_continuation")):
            score += 0.08; reasons.append("SCALP_RECLAIM_CONT")
        if bool(ctx.get("scalp_reversal_trigger")):
            _trig_score = _coerce_float(ctx.get("scalp_reversal_trigger_score"), None)
            _bonus = 0.10
            if _trig_score is not None:
                _bonus += float(np.clip((_trig_score - 0.50) * 0.25, 0.0, 0.08))
            score += _bonus; reasons.append("SCALP_REVERSAL_TRIGGER")
        if chase_quality in {"CONTROLLED", "STRUCTURED", "FIRST_RECLAIM"}:
            score += 0.06; reasons.append(f"chase={chase_quality}")
        elif chase_quality in {"LATE", "EXTENDED", "WEAK"}:
            score -= 0.12; reasons.append(f"chase={chase_quality}")

    if macd_state in good_macd:
        score += 0.08; reasons.append(f"macd={macd_state}")
    elif macd_state in bad_macd:
        score -= 0.12; reasons.append(f"macd={macd_state}")
    if bool(ctx.get("macd_soft_caution")):
        score -= 0.08; reasons.append("macd_soft_caution")
    if ext_state in {"EXHAUSTED", "STALLING", "EXTENDED", "MOMENTUM_FADE"}:
        score -= 0.16; reasons.append(f"ext={ext_state}")
    elif ext_state in {"CLEAN", "FRESH", "COILED", "NONE"}:
        score += 0.04; reasons.append(f"ext={ext_state}")
    try:
        if phase_conf is not None:
            score += 0.08 * math.tanh((float(phase_conf) - 0.62) / 0.18)
    except Exception:
        pass
    try:
        if pressure is not None:
            score += 0.10 * math.tanh((float(pressure) - 2.0) / 0.9)
    except Exception:
        pass

    # Geometry sanity: absurdly wide or compressed room should reduce trust in raw geometry.
    try:
        rr = float(reward_room_pct) / max(float(failure_room_pct), 1e-9)
        if rr >= 1.35:
            score += 0.04; reasons.append("rr_good")
        elif rr < 0.75:
            score -= 0.08; reasons.append("rr_thin")
        avg_room = (float(reward_room_pct) + float(failure_room_pct)) / 2.0
        if avg_room > 4.5:
            score -= 0.08; reasons.append("wide_geometry")
        elif avg_room < 0.45:
            score -= 0.06; reasons.append("compressed_geometry")
    except Exception:
        pass

    score = float(max(0.0, min(1.0, score)))
    if score >= 0.68:
        profile = "HIGH_TRUST_RAW_LEAN"
    elif score >= 0.46:
        profile = "MEDIUM_TRUST_BLEND"
    else:
        profile = "LOW_TRUST_FRACTIONAL"
    reason = ",".join(reasons[:5]) if reasons else "geometry_only"
    return score, f"{profile}:{reason}"
...
def _adaptive_threshold_engine_trigger_pcts(
    ...
    """Return Adaptive Threshold v2 (gain_pct, loss_pct, source_label) after fill.

    v2 keeps adaptive independent from static engine-specific presets. It uses the
    actual trade geometry plus preserved SCALP/RIDE context to decide whether raw
    geometry is trustworthy, whether to blend, or whether to lean fractional/tight.
    """
    try:
    ...
    # Base fractional behavior from the original adaptive engine. This remains the
    # low-trust fallback, but it no longer owns the entire decision unconditionally.
    if engine == "SCALP":
        reward_eff = 0.42
        failure_tol = 0.46
        base_source = "SCALP"
        ...
        reward_eff = 0.34
        failure_tol = 0.62
        base_source = f"RIDE_PULLBACK[{failure_basis_label}]"
    elif engine == "RIDE":
        reward_eff = 0.47
        failure_tol = 0.52
        base_source = f"RIDE_NONPULLBACK[{failure_basis_label}]"
    else:
        reward_eff = 0.40
        failure_tol = 0.50
        base_source = f"GENERIC[{failure_basis_label}]"
    ...
    # Raw geometry targets are intentionally not 100% of TP0/stop room. They mean:
    # let a high-trust trade work closer to the actual structure, without waiting
    # for absolute max TP0/stop violation every time.
    raw_gain = float(reward_room_pct) * (0.82 if engine == "SCALP" else 0.88)
    raw_loss = float(failure_room_pct) * (0.82 if engine == "SCALP" else 0.90)

    trust, trust_label = _adaptive_threshold_trust_score(lifecycle, float(reward_room_pct), float(failure_room_pct))
    try:
        setattr(lifecycle, "adaptive_threshold_profile", trust_label.split(":", 1)[0])
    except Exception:
        pass

    # Convert trust into a raw-geometry blend. High trust leans raw; low trust
    # mostly keeps the safer fractional behavior. This is the core v2 selector.
    raw_weight = float(max(0.0, min(0.78, (trust - 0.32) / 0.68)))
    if trust < 0.46:
        raw_weight *= 0.35
    elif trust >= 0.68:
        raw_weight = max(raw_weight, 0.58)

    gain_pct = (1.0 - raw_weight) * float(fractional_gain) + raw_weight * float(raw_gain)
    loss_pct = (1.0 - raw_weight) * float(fractional_loss) + raw_weight * float(raw_loss)

    # Context-sensitive sanity rails derived from geometry itself, not static engine presets.
    # They prevent adaptive from becoming absurdly tiny or absurdly loose while preserving
    # a truly adaptive path independent of engine-specific fixed thresholds.
    gain_floor = min(float(reward_room_pct) * 0.72, max(0.18, float(reward_room_pct) * 0.22))
    loss_floor = min(float(failure_room_pct) * 0.72, max(0.16, float(failure_room_pct) * 0.24))
    gain_ceiling = max(gain_floor, float(reward_room_pct) * (0.92 if trust >= 0.68 else 0.78 if trust >= 0.46 else 0.62))
    loss_ceiling = max(loss_floor, float(failure_room_pct) * (0.96 if trust >= 0.68 else 0.78 if trust >= 0.46 else 0.60))

    # Extra protection: late/chase/soft-caution contexts should not inherit wide raw stop room.
    ctx = _adaptive_threshold_context(lifecycle)
    late_context = bool(
        str(ctx.get("phase") or "") in {"MATURE_ACCEPTANCE", "EXTEND_THEN_PULLBACK", "STALLING_EXTENSION", "FAILED_EXTENSION"}
        or str(ctx.get("extension_state") or "") in {"EXHAUSTED", "STALLING", "EXTENDED", "MOMENTUM_FADE"}
        or bool(ctx.get("macd_soft_caution"))
        or str(ctx.get("scalp_chase_quality") or "") in {"LATE", "EXTENDED", "WEAK"}
    )
    if late_context:
        loss_ceiling = min(loss_ceiling, float(failure_room_pct) * 0.58)
        gain_ceiling = min(gain_ceiling, float(reward_room_pct) * 0.58)

    gain_pct = float(max(gain_floor, min(gain_pct, gain_ceiling)))
    loss_pct = float(max(loss_floor, min(loss_pct, loss_ceiling)))

    if gain_pct <= 0 or loss_pct <= 0:
        return None
    source = f"ADAPTIVE_V2_{base_source}|trust={trust:.2f}|raw_w={raw_weight:.2f}|{trust_label}"
...
def _in_exec_window(now: datetime, cfg: AutoExecConfig) -> bool:
    """Three windows: 09:40–09:50, 09:50–11:00, and 14:00–15:30 ET.
...
def stage_from_payload(cfg: AutoExecConfig, engine: str, payload: Dict[str, Any], stage: str = "STAGED") -> Optional[TradeLifecycle]:
    ...
    # Preserve selected engine-context fields for Adaptive Threshold v2. These are
    # internal lifecycle breadcrumbs only; they do not change alert payload shape.
    def _nested_payload_get(*keys: str, default=None):
        sources = [payload]
        try:
        ...
        try:
            ...
            if isinstance(raw, dict):
                sources.append(raw)
                rex = raw.get("Extras") if isinstance(raw.get("Extras"), dict) else None
                if isinstance(rex, dict):
                    sources.append(rex)
        except Exception:
            pass
        for src in sources:
            val = _pget(src, *keys, default=None)
            if val not in (None, ""):
                return val
        return default

    structure_phase_detail_meta = str(_nested_payload_get("structure_phase_detail", "StructurePhaseDetail", "phase_detail", default="") or "").upper().strip() or None
    structure_phase_confidence_meta = _parse_float(_nested_payload_get("structure_phase_confidence", "StructurePhaseConfidence", "phase_confidence", default=None))
    macd_momentum_state_meta = str(_nested_payload_get("macd_momentum_state", "macd_state", "MACDState", default="") or "").upper().strip() or None
    macd_soft_caution_meta = _coerce_bool(_nested_payload_get("macd_soft_caution", "MACDSoftCaution", default=False), False)
    breakout_proximity_bucket_meta = str(_nested_payload_get("breakout_proximity_bucket", "BreakoutProximityBucket", default="") or "").upper().strip() or None
    scalp_reclaim_continuation_meta = False
    scalp_reversal_trigger_meta = False
    pressure_score_meta = None
    extension_state_meta = None
    scalp_chase_quality_meta = None
    scalp_reversal_trigger_type_meta = None
    scalp_reversal_trigger_score_meta = None
    try:
        bias_u = str(_pget(payload, "bias", "Bias", "BIAS", default="") or "").upper()
        if engine == "SCALP":
            if "SHORT" in bias_u:
                scalp_reclaim_continuation_meta = _coerce_bool(_nested_payload_get("scalp_reclaim_continuation_short", default=False), False)
                scalp_reversal_trigger_meta = _coerce_bool(_nested_payload_get("scalp_reversal_trigger_short", default=False), False)
                scalp_reversal_trigger_type_meta = str(_nested_payload_get("scalp_reversal_trigger_type_short", default="") or "").upper().strip() or None
                scalp_reversal_trigger_score_meta = _coerce_float(_nested_payload_get("scalp_reversal_trigger_score_short", default=None), None)
                ps = _nested_payload_get("pressure_states", default=None)
                if isinstance(ps, dict):
                    pressure_score_meta = _coerce_float(ps.get("short_pressure_score"), None)
            else:
                scalp_reclaim_continuation_meta = _coerce_bool(_nested_payload_get("scalp_reclaim_continuation_long", default=False), False)
                scalp_reversal_trigger_meta = _coerce_bool(_nested_payload_get("scalp_reversal_trigger_long", default=False), False)
                scalp_reversal_trigger_type_meta = str(_nested_payload_get("scalp_reversal_trigger_type_long", default="") or "").upper().strip() or None
                scalp_reversal_trigger_score_meta = _coerce_float(_nested_payload_get("scalp_reversal_trigger_score_long", default=None), None)
                ps = _nested_payload_get("pressure_states", default=None)
                if isinstance(ps, dict):
                    pressure_score_meta = _coerce_float(ps.get("long_pressure_score"), None)
            # Keep CHASE quality conservative unless the engine explicitly marked a reclaim-continuation.
            em = str(_nested_payload_get("entry_model", "EntryModel", default="") or "").upper()
            if "CHASE" in em and (scalp_reclaim_continuation_meta or scalp_reversal_trigger_meta):
                scalp_chase_quality_meta = "FIRST_RECLAIM" if scalp_reclaim_continuation_meta else "STRUCTURED"
            elif "CHASE" in em:
                scalp_chase_quality_meta = "LATE"
            elif scalp_reversal_trigger_meta:
                scalp_chase_quality_meta = "STRUCTURED"
            elif scalp_reclaim_continuation_meta:
                scalp_chase_quality_meta = "STRUCTURED"
        elif engine == "RIDE":
            rps = _nested_payload_get("ride_pressure_states", default=None)
            if isinstance(rps, dict):
                if "SHORT" in bias_u:
                    pressure_score_meta = _coerce_float(rps.get("short_pressure_score"), None)
                else:
                    pressure_score_meta = _coerce_float(rps.get("long_pressure_score"), None)
            ext = _nested_payload_get("extension_profile", default=None)
            if isinstance(ext, dict):
                if _coerce_bool(ext.get("exhausted"), False):
                    extension_state_meta = "EXHAUSTED"
                elif _coerce_bool(ext.get("stalling"), False):
                    extension_state_meta = "STALLING"
                elif _coerce_bool(ext.get("extended"), False):
                    extension_state_meta = "EXTENDED"
            if not extension_state_meta:
                if _coerce_bool(_nested_payload_get("breakout_exhausted", default=False), False):
                    extension_state_meta = "EXHAUSTED"
                elif _coerce_bool(_nested_payload_get("breakout_stalling", default=False), False):
                    extension_state_meta = "STALLING"
                elif _coerce_bool(_nested_payload_get("breakout_extended", default=False), False):
                    extension_state_meta = "EXTENDED"
                elif _coerce_bool(_nested_payload_get("breakout_momentum_fade", default=False), False):
                    extension_state_meta = "MOMENTUM_FADE"
                else:
                    extension_state_meta = "CLEAN"
    except Exception:
        pass
    if engine == "RIDE":
        ride_entry_mode = str(
    ...
    return TradeLifecycle(
        ...
        structure_phase_detail=structure_phase_detail_meta,
        structure_phase_confidence=structure_phase_confidence_meta,
        macd_momentum_state=macd_momentum_state_meta,
        macd_soft_caution=bool(macd_soft_caution_meta),
        pressure_score=pressure_score_meta,
        extension_state=extension_state_meta,
        breakout_proximity_bucket=breakout_proximity_bucket_meta,
        scalp_reclaim_continuation=bool(scalp_reclaim_continuation_meta),
        scalp_reversal_trigger=bool(scalp_reversal_trigger_meta),
        scalp_reversal_trigger_type=scalp_reversal_trigger_type_meta,
        scalp_reversal_trigger_score=scalp_reversal_trigger_score_meta,
        scalp_chase_quality=scalp_chase_quality_meta,
...
def try_send_entries(cfg: AutoExecConfig, allow_opening: bool, allow_midday: bool, allow_power: bool, fetch_last_price_fn) -> None:
    ...
    for symbol, lst in list(state.get("lifecycles", {}).items()):
        for idx, raw in enumerate(list(lst)):
            ...
            # Structured SCALP reversal triggers need a small, explicit entry-flex window.
            # Without this, the engine can correctly detect the hammer/engulf/sweep trigger,
            # stage at the trigger close, and then miss the fill because price never retouches
            # that exact close. This is NOT generic chase permission: it only applies to the
            # dedicated SCALP reversal trigger flag and is hard-capped in bps.
            scalp_rev_entry_flex_ok = False
            scalp_rev_entry_cap = None
            scalp_rev_flex_bps = 0.0
            try:
                if str(getattr(lifecycle, "engine", "") or "").upper() == "SCALP" and bool(getattr(lifecycle, "scalp_reversal_trigger", False)):
                    scalp_rev_flex_bps = max(0.0, min(float(getattr(cfg, "scalp_reversal_entry_flex_bps", 40.0) or 0.0), 75.0))
                    scalp_rev_entry_cap = float(lifecycle.desired_entry) * (1.0 + scalp_rev_flex_bps / 10000.0)
                    scalp_rev_entry_flex_ok = bool(last > lifecycle.stop and last <= scalp_rev_entry_cap)
            except Exception:
                scalp_rev_entry_flex_ok = False
                scalp_rev_entry_cap = None
                scalp_rev_flex_bps = 0.0
            ...
            if entry_mode == "immediate_on_stage":
                ...
                band_ok = bool(last > lifecycle.stop and last <= float(lifecycle.desired_entry) * band_mult)
                if not (band_ok or scalp_rev_entry_flex_ok):
                    lifecycle.last_entry_eval = (
                        ...
                        f"scalp_rev_flex_bps={scalp_rev_flex_bps} scalp_rev_cap={scalp_rev_entry_cap}"
                    )
                ...
                # touch_required
                # Exception: structured SCALP reversal triggers can use the explicit flex window,
                # otherwise touch_required would make the new trigger lane mostly observational.
                if not ((last <= lifecycle.desired_entry and last > lifecycle.stop) or scalp_rev_entry_flex_ok):
                    lifecycle.last_entry_eval = (
            ...
            try:
                ...
                if scalp_rev_entry_flex_ok and scalp_rev_entry_cap is not None and last > entry_limit:
                    # Marketable-but-capped BUY limit for structured SCALP reversal triggers.
                    # Cap keeps us from paying beyond the explicit trigger-flex tolerance.
                    flex_limit = min(float(last), float(scalp_rev_entry_cap))
                    entry_limit = float(_tick_round(flex_limit) or flex_limit)
                    if entry_limit <= float(lifecycle.stop):
                        lifecycle.last_entry_eval = (
                            f"SKIP: scalp_reversal_flex_violates_stop entry_limit={entry_limit} stop={lifecycle.stop}"
                        )
                        lst[idx] = asdict(lifecycle)
                        continue
...
""",
            ...
            if bool(getattr(cfg, "threshold_exit_enabled", False)):
                ...
                # Adaptive v2 is independent of the engine-specific toggle. Use the
                # same adaptive threshold resolver here that reconcile uses so the
                # first activation email reports the actual threshold policy/values.
                try:
                    _adaptive_thr = _adaptive_threshold_engine_trigger_pcts(cfg, lifecycle, entry_px_dbg)
                    if _adaptive_thr is not None:
                        _thr_gain, _thr_loss, _thr_src = _adaptive_thr
                except Exception:
                    pass
        ...
        try:
            if lifecycle.stage == "IN_POSITION" and bool(getattr(cfg, "threshold_exit_enabled", False)) and _effective_exit_mode(lifecycle) == "THRESHOLD":
                ...
                if (shared_last_px is None) or (not entry_px) or (entry_px <= 0):
                    ...
                    try:
                        use_engine_specific = bool(getattr(cfg, "threshold_exit_use_engine_specific", False))
                        adaptive_policy_enabled = bool(getattr(cfg, "threshold_exit_use_adaptive_engine_policy", False))
                        if (use_engine_specific or adaptive_policy_enabled) and str(getattr(lifecycle, "engine", "") or "").upper() == "RIDE" and str(getattr(lifecycle, "ride_entry_mode", "") or "").upper() == "PULLBACK":
                            pbl = _coerce_float(getattr(lifecycle, "pullback_band_low", None), None)
                            pbh = _coerce_float(getattr(lifecycle, "pullback_band_high", None), None)
                            if pbl is not None and pbh is not None:
